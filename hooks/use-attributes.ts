...
export function useAttributesByEntityType(entityType: "person" | "event" | "place") {
  return useQuery({
...
export function useAddAttribute() {
  ...
  return useMutation({
    ...
    onMutate: async ({ name, category, entityType = "all" }) => {
      ...
      const previousAttributes = queryClient.getQueryData<Attribute[]>(["attributes"])
      const previousEntityAttributes = queryClient.getQueryData<Attribute[]>(["attributes", entityType])
      ...
      if (entityType !== "all") {
        queryClient.setQueryData<Attribute[]>(["attributes", entityType], (old) =>
          old ? [optimisticAttribute, ...old] : [optimisticAttribute],
        )
      }
    ...
    onError: (err, { entityType }, context) => {
      ...
      if (context?.previousEntityAttributes && entityType !== "all") {
        queryClient.setQueryData(["attributes", entityType], context.previousEntityAttributes)
      }
    },
    onSettled: () => {
...
export function useSearchAttributes() {
  return useMutation({
    mutationFn: ({ query, entityType }: { query: string; entityType?: "person" | "event" | "place" }) =>
